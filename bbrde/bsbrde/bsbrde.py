...
LagrangeInterpolator.interpolate(ub_in, u_out) # interpolates u_out onto ub_in
...
# functional forms
Finner  = (u_in - un_in)/dt*v_in*dx_in     + inner(grad(u_in), grad(v_in))*dx_in    - (ub_in  - u_in )*v_in*ds_in        
Fouter  = (u_out - un_out)/dt*v_out*dx_out + inner(grad(u_out), grad(v_out))*dx_out - (ub_out - u_out)*v_out*ds_out(4) 

# solver loop
vtkin   = File("bsb_u_in.pvd")
vtkout  = File("bsb_u_out.pvd")
vtkin  << (u_in, 0)
vtkout << (u_out, 0)
